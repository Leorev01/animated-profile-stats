export default async function handler(req, res) {
  ...
  const { username = "octocat", type = "normal" } = req.query;

  // Type data for each type (copied from test-server.js)
  const typeMap = {
    normal: {
  ...
  try {
    ...
    if (!typeData) {
      res.status(400).send(`<svg xmlns='http://www.w3.org/2000/svg' width='400' height='120'><text x='20' y='60' font-size='20' fill='red'>Invalid type: ${type}</text></svg>`);
      return;
    }
    ...
    // Fetch and embed avatar as base64
    let avatarBase64 = '';
    try {
