...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (typeIcon && typeData) {
    // Update the img src to load the correct SVG file
    const iconImg = typeIcon.querySelector('img');
    if (iconImg) {
      iconImg.src = `assets/types/${userType}.svg`;
      iconImg.alt = `${userType.charAt(0).toUpperCase() + userType.slice(1)} Type`;
    }
    
    // Set background gradient
    const gradient = `radial-gradient(circle at 60% 40%, ${typeData.bgStops[0]}, ${typeData.bgStops[1]})`;
    typeIcon.style.background = gradient;
    
    // Set dynamic shadow for each type
    let shadow = `0 0 8px 2px ${typeData.shadowColor}, 0 2px 8px ${typeData.shadowColor}`;
  ...
  if (card && typeData) {
    ...
    // Set dynamic shadow for each type
    let cardShadow = `0 8px 32px 0 ${typeData.shadowColor}, 0 2px 8px 0 ${typeData.shadowColor}`;
    card.style.setProperty('--card-shadow', cardShadow);
    
    // Set text color
    card.style.color = typeData.textColor;
    // Also update all child text elements for best effect
    card.querySelectorAll('h3, p, b, span, ul, li').forEach(el => {
      el.style.color = typeData.textColor;
    });
  }
  
  // If username is provided, fetch and display user data
  if (username) {
    try {
      await renderDevCard(username, 'dev-card-container');
    } catch (error) {
      ...
      if (card) {
        card.innerHTML = `
          <div style="text-align: center; padding: 20px;">
            ...
            <p>${error.message}</p>
            <p>This API is subject to GitHub public rate limits.</p>
          </div>
        `;
      }
    }
...
async function fetchUserInfo(username) {
  ...
  if (!res.ok) {
    ...
    console.log(`GitHub API error for user ${username}: ${res.status} - ${errorText}`);
    if (res.status === 404) {
...
async function renderDevCard(username, containerId) {
  const headers = {
    'User-Agent': 'animated-profile-stats'
  };
  ...
  try {
    ...
    const events = await fetchRecentActivity(username);
    ...
    // 6) Get top languages
    const topLangs = await getTopLangs(username, headers);
  ...
  document.getElementById('dev-name').textContent = user.name || user.login;
    document.getElementById('dev-exp').textContent = level;
    document.getElementById('dev-skills').textContent = topLangs;
